...
function VT_speechTraining_10AFC_experiment(name, exptdesign, stimType)
...
%         'Press any key to continue'],1)
    ...
    if stimType == 1
        load('stimuli_Gen2_10AFC.mat');
    elseif stimType == 2
        load('stimuli_FB_10AFC.mat');
    end
    ...
    for iBlock=1:exptdesign.numSessions %how many blocks to run this training session
        ...
        for iTrial=1:exptdesign.numTrialsPerSession
            ...
            if accuracy==0
                ...
                drawAndCenterText(w, ['Incorrect. The correct answer was ' target '. Press any key to continue.'], 0);
                while KbCheck; end % Wait until all keys are released.
                KbWait
                %get keyboard response
            else
                drawAndCenterText(w, ['Correct! The correct answer was ' target '. Press any key to continue.'], 0);
                while KbCheck; end % Wait until all keys are released.
                KbWait
                %get keyboard response
...
%             trialOutput(iBlock).RespMissed(iTrial)=RespMissed;
            ...
            elseif iTrial==exptdesign.numTrialsPerSession && iBlock == exptdesign.numSessions
                ...
                accuracyForBlock=mean(trialOutput(iBlock).accuracy);
                drawAndCenterText(w, ['Your accuracy was ' num2str(round(accuracyForBlock.*100)) '%\n\n\n'...
                    'You have completed this training session.  Thank you for your work!' ],0) % was a 1
                %KbWait(1)
                while KbCheck; end % Wait until all keys are released.
                KbWait
                Screen('CloseAll')
