function makeGen2TrainingStim

nStimPerBlock = 60;

tmp = importdata('stimuli\trainingset-editdistance-percent-correct.csv');
dm = tmp.data;
load('stimuli\wordlist_train_percent.mat');
...
%        load(['stimuli/Gen2_raw/' wordlist{i(4)} '.mat']);
%        stim5{l}{w,9} = rawData;
%        stim5{l}{w,10} = rawData;
%        load(['stimuli/Gen2_raw/' wordlist{i(5)} '.mat']);
%        stim5{l}{w,11} = rawData;
%        stim5{l}{w,12} = rawData;
%        load(['stimuli/Gen2_raw/' wordlist{i(6)} '.mat']);
%        stim5{l}{w,13} = rawData;
%        stim5{l}{w,14} = rawData;
%        load(['stimuli/Gen2_raw/' wordlist{i(7)} '.mat']);
%        stim5{l}{w,15} = rawData;
%        stim5{l}{w,16} = rawData;
%        k=k+1;
%        if k==30, k=1; end
%    end
    %duplicate stimuli
...
save('stimuli_Gen2_amp','stim','label')
