...
function VT_speechTraining_experiment_Gen2_v2(name, exptdesign, stimType)
...
%         'Press any key to continue'],1)
    ...
    if stimType == 1
        makeGen2TrainingStim;
        load('stimuli_Gen2.mat');
    elseif stimType == 2
        makeFBTrainingStim;
        load('stimuli_FB.mat');
    end
    ...
    for iBlock=1:exptdesign.numSessions %how many blocks to run this training session
      ...
      %  if iBlock ==1
%             drawAndCenterText(w,['Before starting training, you feel and hear a series of meaningless syllables. '\n\n\n\n'...
%                 'Compare how the syllables feel on your arm with how they sound and see if you can identify any similarities. \n\n\n\n'...
%                 'Press any key to continue'],0);
%             KbWait
...
%         end
        for iTrial=1:exptdesign.numTrialsPerSession
            ...
            rtn_load = piezoDriverGen2('loadGesture',eventFile);
                        ...
                        for i=1:size(labels,2)
                            try 
                                if strcmp(labels{iTrial,i},disp_labels{str2num(sRespFb(1))})
                                    eventFile = stimuli{iTrial,i+(i-1)};
                                    %rawGesture = gen2ConvertGesture(eventFile);
...
%             trialOutput(iBlock).RespMissed(iTrial)=RespMissed;
            ...
            elseif iTrial==exptdesign.numTrialsPerSession && iBlock == exptdesign.numSessions
                ...
                accuracyForBlock=mean(trialOutput(iBlock).accuracy);
                drawAndCenterText(w, ['Your accuracy was ' num2str(round(accuracyForBlock.*100)) '%\n\n\n'...
                    'You have completed this training session.  Thank you for your work! Please hit enter.' ],0)% was 1
                %KbWait(1)
                %KbWait(1)
                while KbCheck; end % Wait until all keys are released.
                KbWait
